...
import { useUpdateProject } from "~/services/projects/update";
...
export default function EditProjectForm({ data }: EditProjectFormProps) {
  ...
  const form = useForm<Proyek>({
    ...
    defaultValues: {
      ...
      asset_jaminan: data.asset_jaminan || "",
      nilai_jaminan: data.nilai_jaminan.toString() || "",
      lokasi_usaha: data.lokasi_usaha || "",
      detail_lokasi: data.detail_lokasi || "",
      pendapatan_perbulan: data.pendapatan_perbulan.toString() || "",
      pengeluaran_perbulan: data.pengeluaran_perbulan.toString() || "",
      limit_siklus: data.limit_siklus?.toString() || "",
      bagian_pelaksana: data.bagian_pelaksana?.toString() || "",
      bagian_koperasi: data.bagian_koperasi?.toString() || "",
      bagian_pemilik: data.bagian_pemilik?.toString() || "",
      bagian_pendana: data.bagian_pendana?.toString() || "",
    },
  });
  ...
  async function onSubmit(data: Proyek) {
    try {
      ...
      console.log(error);
      toast.error("Gagal mengupdate proyek.");
    }
  }
